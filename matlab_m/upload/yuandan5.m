...
%% %设定字体和字号
text(0,0.5,"啊","FontSize",100,"FontName","黑体");
...
%% 设定填充
mystrings={'Sherry'}; %填充字符

%% 开始填充
f=figure;
axis equal
axis([1 2*size(BW,2) 1 2*size(BW,1)])
hold on
for i=1:size(BW,1)
    for j=1:size(BW,2)
        if BW(i,j)==0
            text(2*j,2*(size(BW,1)-i),mystrings{randi([1,length(mystrings)])},'Color',[rand*0.2+0.4 rand*0.2+0.3 rand*0.2+0.3],'FontSize',10); %绘制字符
            pause(0.0001)
        end
    end
end
